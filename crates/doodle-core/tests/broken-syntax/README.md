...
cataloged as-is. **The held parser span fixes are now unblocked** by this
sign-off.
...
|---|---|---|---|---|---|
| `01-missing-end` | 1 | `syntax-error` | NEEDS-WORK | (b) EOF span; point at / note the unclosed `to greet()` | ok |
...
| `08-missing-fn-end` | 1 | `syntax-error` | NEEDS-WORK | (b) EOF span (identical defect to 01); point at the opening `fn` | ok |
...
| `16-unbalanced-paren` | 1 | `syntax-error` | NEEDS-WORK | (b) EOF span; the missing `)` belongs at end of line 1, not the blank line | ok |
...
| `26-duplicate-declaration` | 1 | `duplicate-declaration` | NEEDS-WORK | (b)(c) add a note at the first `let a = 1`; suggest rename/drop the second | ok |
...
| `32-missing-index-bracket` | 1 | `syntax-error` | NEEDS-WORK | (b) EOF span; point after `0`, not the blank next line | ok |
...
| `35-record-missing-end` | 1 | `syntax-error` | NEEDS-WORK | (b) EOF span; point at the opening `record` | ok |
...
## Corpus additions (approved with the 2026-07-19 sign-off; rows 42–45 pending)

The individually-ruled interpolation/escape diagnostics are exactly what this
corpus exists to review, and are absent. Add (programs + snapshots + table
rows; the diagnostics already exist from M1.4):

- `42-empty-interpolation` — `{}` in a string (S-48's dual-intent message).
- `43-comment-in-interpolation` — `#` inside `{…}` (S-50's targeted error).
...
- `45-short-hex-escape-string` — string-side `\x` with one digit (S-49's
  malformed-escape; `41` covers only the bytes-literal side).
...
plus the additions above.** M1.13's review gate is met once the span fixes
(systemic finding 1, in scope) land and rows 42–45 join the table; every
diagnostic must point at the correct span.
