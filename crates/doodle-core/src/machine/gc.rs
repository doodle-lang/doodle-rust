...
//! any in-flight unwind, the tail-elided ring, and every loaded module's namespace — and
//! seeds each heap reference it finds into the heap's precise collector
...
//! callables, the **host handle table** (§16, M2a.11), and **every loaded module's
//! namespace binding cells** (`machine.module_root_cells`, AD5 — a module is a singleton
//! that lives for the instance, so its globals are permanent roots), plus a **parked
//! capability request**'s bound arguments while `Suspended` (§14, M2b.4). A frame's
//! `block_param`, `Block` static link, and `Consumer` hold frame indices/serials — not
//! heap references — so they root nothing. Also the in-flight synchronous foreign-call
//! arguments (§15) and the **dynamic-binding save stack** (§13 — each `with`'s saved prior
//! value). Roots that join later as their state lands: the drive stack (M2b.5).
...
pub(crate) fn collect(heap: &mut Heap, machine: &Machine) {
    heap.collect(|tracer| {
        ...
        // Every loaded module's namespace cells (AD5): permanent roots for the instance's
        // life, so a collection during any module's step keeps all modules' globals alive.
        for &cell in &machine.module_root_cells {
            tracer.cell(cell);
        }
        // The exception each `failed` module retains (S-8): kept reachable so a re-import
        // (M9b reload) can re-raise the very value the original load produced.
        ...
        // nothing.
        if let Some(super::modload::Suspension::Capability(pending)) = &machine.pending {
