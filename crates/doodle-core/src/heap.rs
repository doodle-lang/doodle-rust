...
impl Heap {
    ...
    /// Overwrites the value of the record's field at `pos` (a field place assignment,
    /// L§5.3). No charge — one value width replaces another (like
    /// [`dict_set_value`](Self::dict_set_value), the accounting-integrity reason there
    /// is no raw `&mut RecObj`). The caller resolves `pos` from the type's field order.
    pub fn record_set_field(&mut self, idx: RecIdx, pos: usize, value: Value) {
        self.records.get_mut(idx.0).fields[pos] = value;
    }
